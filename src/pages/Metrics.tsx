...
const flowPerformanceData = [
  { name: "Commercial Property", executions: 847, success: 98.2, time: 3.2 },
  { name: "Auto Policy Renewal", executions: 1234, success: 99.1, time: 1.8 },
  { name: "Life Insurance Eval", executions: 456, success: 96.5, time: 4.5 },
  { name: "Workers Compensation", executions: 678, success: 97.8, time: 2.1 },
  { name: "Marine Cargo", executions: 234, success: 95.4, time: 5.3 },
];

const agentPerformanceData = [
  { name: "Document Review Agent", model: "GPT-4.1", latency: "1.2s", accuracy: "98.4%", calls: 2341 },
  { name: "Risk Classification Agent", model: "Claude Opus", latency: "2.1s", accuracy: "97.8%", calls: 1876 },
  { name: "Premium Calculation Agent", model: "GPT-4.1 Mini", latency: "0.8s", accuracy: "99.2%", calls: 1654 },
  { name: "Fraud Detection Agent", model: "GPT-4.1", latency: "1.8s", accuracy: "96.5%", calls: 1432 },
  { name: "Approval Routing Agent", model: "Gemini Pro", latency: "0.6s", accuracy: "99.5%", calls: 1287 },
];
...
const businessImpactMetrics = [
  ...
  {
    label: "Avg Processing Time Saved",
    value: "4.2 hrs",
    change: "+18%",
    trend: "up",
    icon: Clock,
...
const Metrics = () => {
  return (
    <div className="p-8 bg-gradient-to-br from-background via-background to-primary/[0.02] min-h-screen">
      ...
      {/* SECTION 3: BUSINESS IMPACT (Moved to top for executive visibility) */}
      <motion.div
      ...
      >
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-section-title">Business Impact</h2>
        </div>
          ...
          {businessImpactMetrics.map((metric, index) => {
            ...
            return (
              <motion.div
                ...
                className="card-float p-6 relative overflow-hidden group"
              >
                {/* Ambient glow */}
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="w-11 h-11 bg-gradient-to-br from-primary/15 to-primary/5 rounded-xl flex items-center justify-center">
      ...
      >
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-section-title">Flow Performance</h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div
            ...
            className="card-float p-6 lg:col-span-2"
          >
          ...
          >
            <h3 className="text-sm font-medium text-foreground mb-4">Policy Type Distribution</h3>
            <div className="h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left text-table-header py-3 pr-4">Flow Name</th>
                  <th className="text-right text-table-header py-3 px-4">Executions</th>
                  <th className="text-right text-table-header py-3 px-4">Success Rate</th>
                  <th className="text-right text-table-header py-3 pl-4">Avg Time</th>
                </tr>
              </thead>
              <tbody>
                {flowPerformanceData.map((flow, index) => (
                  ...
                  >
                    ...
                    <td className="text-right py-4 px-4">
                      <span className="text-sm text-foreground">{flow.executions.toLocaleString()}</span>
                    </td>
                    <td className="text-right py-4 px-4">
                      <span className={`text-sm font-medium ${flow.success >= 98 ? 'text-success' : flow.success >= 96 ? 'text-warning' : 'text-error'}`}>
                        {flow.success}%
                      </span>
                    </td>
                    <td className="text-right py-4 pl-4">
                      <span className="text-sm text-muted-foreground">{flow.time} min</span>
                    </td>
                  </motion.tr>
                ))}
              </tbody>
      ...
      >
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-section-title">Agent Performance</h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="space-y-3">
            {agentPerformanceData.slice(0, 4).map((agent, index) => (
              <motion.div
                key={agent.name}
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-primary/15 to-primary/5 rounded-xl flex items-center justify-center group-hover:from-primary/25 transition-colors">
                    <Brain className="w-5 h-5 text-primary" />
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    ...
                    <div className="text-sm font-medium text-foreground">{agent.latency}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-muted-foreground">Accuracy</div>
                    <div className="text-sm font-medium text-success">{agent.accuracy}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-muted-foreground">Calls</div>
                    <div className="text-sm font-medium text-foreground">{agent.calls.toLocaleString()}</div>
                  </div>
                </div>
              </motion.div>
            ))}
